...
export class FileSystem {
    ...
    public static fileNameUsingTextDocument(document: vscode.TextDocument, withExtension = true): string {
        return path.basename(document.fileName, withExtension ? undefined : path.extname(document.fileName));
    }

    public static fileNameUsingUri(uri: vscode.Uri, withExtension = true): string {
        return path.basename(uri.fsPath, withExtension ? undefined : path.extname(uri.fsPath));
    }

    public static async makeDirAsync(thePath: string, pathIsfile = false): Promise<void> {
    ...
    public static async writeFileAsync(filePath: string, dataOrObject: any, overwrite = false, includeNullUndefinedOnStringify = true): Promise<void> {
        return new Promise<void>(async (resolve, reject) => {
